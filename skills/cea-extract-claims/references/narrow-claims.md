...
A paper can have no broad statements at all, when every main result it states is qualitative. Record the rejected candidates, record no claims, and say so in the report.
...
A model's accuracy is a rejected candidate when the paper only reads other findings from the model, such as feature importances or labels. It is a claim candidate when the paper presents the model as a contribution, or when a research question asks how well the outcome can be predicted. Of several accuracy figures for one model, select the figures that the main result's wording rests on, not figures per class, per subset, or for models that the paper compared only incidentally. Of several figures for the setup that the study used, select the figure for the outcome that the later analysis rests on, and record the other figures as rejected candidates that name the selected figure.
