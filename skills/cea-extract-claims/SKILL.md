...
- a rejected candidate's `reason` that names neither a ground nor a broad statement, such as "Important result", or that says no main result depends on the candidate although the record has broad statements
- broad statements that most of the same claims serve, which read as one main result stated twice
...
Recording a broad statement as a rejected candidate instead leaves every claim that served it naming an id that no longer exists, which is a problem rather than a warning, so judge those claims again under item 3 of step 5 first.
...
The render command writes `claims.md`. The claims stand under each main result they serve, the result the paper states in the most places first, and within a result in page order, because the paper puts no order on the claims that support one result. Broad statements that most of the same claims serve are shown together as one result, which usually means that the paper states one result twice, so check that grouping against the quotes. A list of every claim in page order and the rejected candidates come next. A statement that step 7 reports as a breakdown still counts here as a place that states the result; the warning decides, not the grouping. Then tell the user, briefly:
