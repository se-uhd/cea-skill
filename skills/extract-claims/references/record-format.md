...
- `paper.doi` (optional): the paper's DOI, such as `10.1145/1083142.1083147`, without `https://doi.org/` in front; the page makes the link. Take it from the paper's first page, from the publisher, or from the checker, never from elsewhere in the paper's text, where a DOI names a cited work or a replication package. Leave the field out rather than guess one. It is how a reader reaches a paper whose PDF is not published beside its page, which `site` describes.
...
- A quote whose own lines in `text.txt` read as a row of a table rather than as a sentence draws a warning on a claim. A number that stands only in a table is evidence for a claim, not a claim itself. An excluded claim candidate may quote such a row, which is the rule being followed.
...
- `note` (optional, on any entry): only facts that the checker can confirm in the paper, each with its location. A note can say:
  ...
  - which main result or passage might depend on an excluded claim candidate
...
- an excluded claim candidate's `reason` that runs under three words, or gives a verdict instead of a ground, such as "Important result", or that says no main result depends on the candidate although the record has main results
